...
x = np.linspace(-2.3, 2.3, 1000)  # Adjust the range as needed
...
plt.hist(data, label='histogram',bins=30, density=True, alpha=0.6, color='b')
plt.plot(x, pdf, label='PDF', color='r')
plt.plot(x, pdf_interpolated, label='Interpolated PDF', color='g', linestyle='--')
plt.plot(x, normal_pdf(x, mu_fit, sigma_fit), 'cyan', label='Fitted Normal')
plt.xlabel('X')
